...
const HomePage = () => {
  ...
  useEffect(() => {
    if (me._id) {
      if (!done && socket.id && me._id && socket) {
  ...
  useEffect(() => {
    try {
      ...
      if (!me._id) {
        fetchData(`${baseurl}/api/v1/users/me`).then((res) => {
          try {
            setMe(res.user);
      ...
      if (!section) {
        ...
        if (story.length === 0) {
          fetchData(`${baseurl}/api/v1/stories`).then((res) => {
            try {
              setStory(res.doc);
            } catch (err) {
              showAlert({
        ...
        if (suggestions.length === 0) {
          fetchData(`${baseurl}/api/v1/users/findsuggestions`).then((res) => {
            try {
        ...
        fetchData(`${baseurl}/api/v1/users/findPosts`).then((res) => {
          try {
        ...
        fetchData(`${baseurl}/api/v1/users/findStories`).then((res) => {
          try {
            setMyStory(res.doc);
          } catch (err) {
            showAlert({
        ...
        fetchData(`${baseurl}/api/v1/users/${id}`).then((res) => {
          try {
        ...
        fetchData(`${baseurl}/api/v1/users/findPosts/${id}`).then((res) => {
          try {
            setMyPosts(res.posts);
          } catch (err) {
            showAlert({
        ...
        setShowPost(true);
        fetchData(`${baseurl}/api/v1/posts/${id}`).then((res) => {
          try {
            setEachPost(res.doc);
          } catch (err) {
            showAlert({
      ...
      content = (
        <div>
          ...
          {callReceiving === true && caller._id ? (
            <div className="pt-[10%]">
              {/* Wrap both image and animation within the same parent div */}
