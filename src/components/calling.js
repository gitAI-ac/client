...
function VideoCall() {
  ...
  console.log(sender, receiver, which, senderid, receiverid, camera);
  ...
  useEffect(() => {
    try {
      fetchData({
        url: `${baseurl}/api/v1/users/${receiverid}`,
        jwtToken,
        showAlert,
      }).then((res) => {
      ...
      showAlert({
        type: 'error',
        message: 'errror fetching profile',
      });
    }
    try {
      fetchData({
        url: `${baseurl}/api/v1/users/${senderid}`,
        jwtToken,
        showAlert,
      }).then((res) => {
        setMyData(res.user);
