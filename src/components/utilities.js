...
export async function updateMessagehandler({
  ...
  try {
    const res = await updateMessage({
      url: `${baseurl}/api/v1/conversations/${id}/${messageid}?`,
      jwtToken: jwtToken,
      showAlert: showAlert,
      seen,
    });
...
export async function handleLiker({
  type,
  id,
  jwtToken,
  body,
  ...
  try {
    axios.post(`${baseurl}/api/v1/${type}/like/${id}`, body, {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
        'Content-Type': 'application/json', // Specify the content type of the request
      },
...
export async function handleCommenting({
  ...
  try {
    return await axios
      .post(
        `${baseurl}/api/v1/posts/${type}/${posttoComment}`,
        type === 'comments'
